...
class StopwatchProvider {
    ...
    get elapsedMilliseconds() {
        ...
        return ((_a = this.stopTime) !== null && _a !== void 0 ? _a : new Date().getTime()) - this.startTime;
    }
    start() {
...
class Clock {
    constructor(canvas, provider = new ClockProvider()) {
        this.canvas = canvas;
    ...
    drawDigit(ctx, hour, minute, second, millisecond) {
        ctx.font = `25px ${FONT}`;
        ctx.fillStyle = TEXT_COLOR;
        ctx.textBaseline = 'middle';
        ctx.textAlign = 'center';
...
(function (StopwatchState) {
    StopwatchState[StopwatchState["OFF"] = 0] = "OFF";
    StopwatchState[StopwatchState["RUNNING"] = 1] = "RUNNING";
    StopwatchState[StopwatchState["STOP"] = 2] = "STOP";
})(StopwatchState || (StopwatchState = {}));
class StopwatchManager {
    constructor(clock, btn) {
        this.state = StopwatchState.OFF;
        this.provider = new StopwatchProvider();
        this.btnMsgs = {
            0: "ストップウォッチ",
            1: "ストップ",
            2: "時計に戻る"
...
const CLOCK_CANVAS = document.getElementById('clockCanvas');
const CLOCK = new Clock(CLOCK_CANVAS);
const STOPWATCH_BTN = document.getElementById('stopwatchButton');
